    ...
    enter: function(node, parent) {
      switch(node.type) {
        ...
        case 'FunctionExpression':
            ...
            if(parent.type === 'CallExpression' && parent.callee.type === 'FunctionExpression'){
              iife.push(filename+"_"+fetchIID(offset));
            }
          break;
...
//check the offset with the json instrumented file for the respective functions IID.
  function fetchIID(o) {
    ...
    function varPropComputations(s, e) {
        var query, calls;
        lastfunc = functionsList[functionsList.length - 1];
        if(iife.indexOf(lastfunc) > -1) return;
        query = { 
    ...
    for(var i=0; i<iife.length; i++) {
      CallertoCallee = _.omit(CallertoCallee, iife[i]);
    }
