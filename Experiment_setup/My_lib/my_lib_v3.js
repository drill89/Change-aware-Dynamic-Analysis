/* A small library to find the percentage of a student in an exam.
   ...
   To obtain the total points obtained by the student for each subject, the average of the best of 2 out of 3 tests is added to the final exam.
...
var ArthimeticOperations = {
    addition : function (arr) { //arr is an array
	var len = arr.length;
	var sum = 0;
        for(var  i=0; i<len; i++) {
	    sum += arr[i];
...
var Subject_points = function(arr, final_points) {
    arr = incremental_sort(arr); //returns the sorted array in incremental order
    arr = array_slice(arr); //returns the last 2 elements of the array   
...
var incremental_sort = function(arr) {
    arr.sort();
    return arr;
}

var array_slice = function(arr) {
    arr = arr.slice(-2);
    return arr;
}
